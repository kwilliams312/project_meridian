...
- **Packaging & CD (D-30, amended D-37 / SYNC §16):** every push to a realm branch (`dev`/`ptr`/`main`) that passes CI **autopublishes** multi-arch (amd64+arm64) daemon images to GHCR — tagged `<short-sha>` plus a moving per-branch tag (`:dev`/`:ptr`/`:prod`; `main` also keeps `:latest`), semver on releases — with digests recorded in the build manifest; CI is the only image producer. Images are multi-stage, non-root, healthchecked, cosign-signed + SBOM-attested at publish. **Compose remains the reference for LOCAL development** and contributor onboarding (build-from-source one flag away). **Hosted realms deploy via ArgoCD + the versioned Helm chart (GitOps):** `dev`/`ptr`/`main` map to the Dev/PTR/Prod realms (single-`worldd` topology today; the sharded chart lands with OPS-04 at M3), config-only consumers of the same images and 12-factor env — no K8s-specific code in the daemons. Semver tags produce GitHub Releases with pinned digests; community realms track releases. See the [CD hosted-realms design](../superpowers/specs/2026-07-07-cd-hosted-realms-design.md).
...
- **Hosted realms (Baseline §6, D-37 / SYNC §16):** the `dev`/`ptr`/`main` branches deploy continuously to the Dev/PTR/Prod realms via **ArgoCD (GitOps)**, replacing the former nightly compose/systemd redeploy. A push to `dev`, and each promotion (`dev → ptr → main`), rolls the new images out; a smoke + concurrency harness (login → enter world → move → kill mob → complete quest, via the headless `meridian-bot` — in-cluster Jobs for scale, or an external NodePort client) validates each realm. Dev uses an ephemeral DB reseeded per run; PTR/Prod persist on Longhorn. A failed smoke fails the promotion; ArgoCD notifications alert the on-call track owner. Rollback is `git revert` on the affected branch.
