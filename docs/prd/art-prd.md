...
- Skeletal: one skeleton per race/sex, shared by all gear. **CHR-01 customization is presets-first (A-03 / D-32):** discrete preset parts — hair, face, skin tone, and class/race-appropriate options — are the primary customization axis and are the bounded set Art authors for M1 (§5: "customization set v1 — 4 hair, 4 face, 3 skin palettes"). Blend shapes (morph targets) carried through glTF are reserved for the **optional 1–2 continuous morphs** (e.g. height/build) that ship **only if the min-spec blend-shape/GPU budget allows** (§2.5 crowd draw-call/animation ceiling is the binding constraint — every extra active morph adds per-character skinning cost in 50+ player scenes). **Deep multi-slider morph customization is deferred post-1.0**; M1 authors a countable preset set, not an open-ended slider rig.
...
5. **CHR-01 customization depth** — **RESOLVED (A-03 / D-32, Sync Decisions §7.2):** presets-first (discrete preset IDs: hair/face/skin + class/race options), plus an **optional 1–2 continuous morphs** only if the blend-shape/GPU budget (§4.2, §2.5) allows; deep morph customization deferred post-1.0. The server persists appearance as a **versioned, extensible appearance record** (preset IDs + optional morph values behind a schema-version tag), so post-1.0 depth is additive without a character-record migration — the per-character customization payload is therefore bounded and known at M1.
