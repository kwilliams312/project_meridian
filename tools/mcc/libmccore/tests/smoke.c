 ...
 *   - pickable  -> asserts itemRef yields count:5 / ok:true
...
int main(void) {
    ...
    check(contains(idx, "\"id_count\": 37"), "index reports id_count 37 (matches mcc index)");
    check(contains(idx, "core:item.kobold_ear"), "index contains core:item.kobold_ear");
    mccore_free(idx);

    /* ---- pickable(itemRef) -> 5 candidates ------------------------------- */
    printf("\n[pickable]\n");
    st = 12345;
    ...
    check(contains(pick, "\"count\": 5"), "pickable itemRef count is 5 (matches mcc pickable)");
    mccore_free(pick);
