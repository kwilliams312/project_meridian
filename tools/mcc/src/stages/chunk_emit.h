...
//       129×129 f32 Heightfield is deliberately NON-FLAT (per the selected
//       HeightProfile — a ramp + shallow bowl, or #876's rolling `meadow`; always
...
// libm calls are sqrt/fabs in the mesh normal path and floor in the `meadow`
// profile's noise lattice — all three are IEEE-754 correctly-rounded (floor is
// exact) and therefore bit-identical across platforms, unlike the transcendentals
// (sin/exp), which stay banned. That ban is why `meadow` is polynomial value
// noise rather than summed sines. Nondeterminism here is a P0 — it breaks pack
// verification.
//
// EMITTING SPROUT MEADOW (#876) — the canonical chibi command; the generator is
// this stage, NOT checked-in data (generated bytes rot and defeat the
// determinism gate, so the pack is re-emitted from source on demand):
//
//   mcc chunk-emit --zone chibi:zone.sprout_meadow --profile meadow \
//                  --grid 3 --origin-x -384 --origin-z -384 --out <dir>
//
// The origin/grid deliberately MATCH Zone-01's geometry (zone-local [-512,-128]),
// so the server's spawn placeholder (kZoneSpawnXY = -320) and its compile-time
// zone-bounds constants keep holding with no server change. Staging the emitted
// pack into client/project/meridian/chibi/ is #877 (T3), not this stage's job.
...
// The named height profile the surface is generated from (#876). The profile is
// the ONLY thing that changes between zones — grid/origin/schema/hashing are
// shared — so a new zone's terrain is a profile, not a new emit path.
//
//   Fixture — the original Zone-01 v0 surface (#553): base 8 m + an eastward
//             ramp + a shallow bowl. Reaches ~+39 m across a 3×3 grid. It was
//             authored to be obviously NON-FLAT for client tests, never to be
...
//    rejects |z − ground| > 4 m, so terrain height IS R5 error budget. `meadow`
//    is amplitude-capped to fit; `fixture` does not fit and must not be shipped
//    as a walkable zone's surface.
enum class HeightProfile {
    Fixture,  // default — the #553 Zone-01 fixture surface (unchanged output)
    Meadow,   // #876 — gentle rolling hills, |height| ≤ 2.6 m
};

// Parse a profile name ("fixture" | "meadow"). Returns false on an unknown name.
...
struct ChunkEmitOptions {
    ...
    // Height profile. Defaults to Fixture so the checked-in Zone-01 golden /
    // staged pack stay byte-identical — `meadow` is strictly opt-in.
    HeightProfile profile = HeightProfile::Fixture;
