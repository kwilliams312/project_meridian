...
int main() {
    ...
    // ===== 4. Replacement update reaches owner + observer ===================
    {
        WorldState world;
        ...
        check("4: visual replacement reaches owner + observer", recipients == 2);
        check("4: owner receives full replacement", owner_items == std::vector<std::uint32_t>{777001});
        check("4: observer receives full replacement", observer_items == std::vector<std::uint32_t>{777001});
        ...
        check("4: late observer EntityEnter carries stored replacement",
